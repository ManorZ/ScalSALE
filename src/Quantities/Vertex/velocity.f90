...
contains
    ...
    subroutine Calculate_derivatives(this, coordinates, total_vof, total_volume, nx, ny, nz, emf)
        ...
        !$omp shared(nz,ny,nx,vof,emf,x,y,z, & 
        !$omp        velocity_x,velocity_y,velocity_z, &
        !$omp        dvel_x_dx,dvel_x_dy,dvel_x_dz,dvel_y_dx,dvel_y_dy,dvel_y_dz,dvel_z_dx,dvel_z_dy,dvel_z_dz, &
        !$omp        vol)
        ...
        !!$omp         u8u7u5,u8u5u4,u8u7u4)
        !!$omp single
        !write(*,*) "velocity:Calculate_derivatives:342: OMP #threads: ", omp_get_num_threads()
        !!$omp end single
        !!$omp do collapse(3)
        do k = 1, nz
            do j = 1, ny
